...
# Help message
usage() {
    echo "SLURM submission script for array jobs with CPU and GPU tasks. Use to run any bash script in parallel."
    ...
    echo "  -c  CPUs per task (default: $DEFAULT_CPUS_PER_TASK). Typically does not effect performance of ChromBPNet pipeline."
    echo "  -m  Memory allocation (default: $DEFAULT_MEM). For deeply sequenced data (e.g. > 100M read pairs), this typically needs to be > 50G"
