...
if __name__=='__main__':
    ...
    num_patterns = len(f['pos_patterns'])-1
    
    for i in range(num_patterns):
        trimmed_ppm = trim_ppm(f['pos_patterns']['pattern_{}'.format(i)]['sequence'], 
                t=args.threshold, min_length=args.min_length, flank=args.flank_add)

        if trimmed_ppm is not None:
            num_seqlets = len(f['pos_patterns']['pattern_{}'.format(i)]['seqlets']['sequence'])
